...
if(!isset($_POST['ventaBoletaSerie'])){
    exit("No se realizó la venta (P) - " + $conn->error);
}

require_once("dbConection.php"); 

//documento
$serie = $conn->real_escape_string($_POST['ventaBoletaSerie']);
$correlativo = $conn->real_escape_string($_POST['ventaBoletaCorrelativo']);
//documento-precios
...
//cliente
$nombre = $conn->real_escape_string($_POST['nombreCliente_venta']);
$tipoDoc = $conn->real_escape_string($_POST['tipoDoc_venta']);
$numDoc = $conn->real_escape_string($_POST['numDoc_venta']);
//tabla
...
//----------------------------crear nueva boleta
$query  = "INSERT INTO Documento(tipoDoc, serie, correlativo, fecha, opGravadas, opExoneradas, montoIGV, clienteDoc, vigente) VALUES('B','".$serie."',".$correlativo.",'".$fecha."',".$opGravadas.",".$opExoneradas.",".$montoIGV.",'".$numDoc."',-1);";
...
//--------------------------seleccionar el codigo de la boleta creada
$query = "SELECT MAX(codigo) AS codigo FROM documento;";

if(!$result = $conn->query($query)){
    echo "No se pudo registrrar venta (obtener codigo maximo) [".$conn->error."]";
    mysqli_rollback($conn);
    exit();
}
...
require_once("greenterFiles/boleta.php");